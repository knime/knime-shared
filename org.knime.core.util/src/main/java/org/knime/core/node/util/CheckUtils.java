...
public final class CheckUtils {
    ...
    /**
     * Checks the given object to be an instance of the given class and if it is, returns it cast. In case the object is
     * null or not an instance of the provided class, the specified exception generating function is called
     * using the provided template arguments.
     * <p>
     * <b>Note:</b> prefer an {@code instanceof} check over using the thrown exception for control flow.
     *
     * @param <T> type of exception thrown
     * @param <V> type to return given object of, if successful
     * @param toCheck object to check
     * @param clazz class object whose type to check for
     * @param toThrowable function to create the thrown exception given the message
     * @param template the string template which determines the exception message
     * @param templateArgs the arguments for the string formating
    ...
    public static <O, T extends Throwable, V> V checkCast(final O toCheck, final Class<V> clazz,
        ...
        final String template, final Object... templateArgs) throws T {
        check(clazz.isInstance(toCheck), toThrowable, stringFormatSupplier(template, templateArgs));
        return clazz.cast(toCheck);
    }

    /**
     * Checks the given predicate to be {@code true}. Otherwise throws an exception generated by the provided function
     * and exception message supplier.
     * <p>
     * <b>Note:</b> prefer the given boolean value over using the thrown exception for control flow.
     *
     * @param <T> type of exception
     * @param predicate predicate to check {@code true}
    ...
    public static <T extends Throwable> void check(final boolean predicate,
        final Function<String, T> toThrowable,
        final Supplier<String> exceptionMessage) throws T {
        if (!predicate) {
            throw toThrowable.apply(exceptionMessage.get());
        }
    }
            ...
            if (localPath != null) {
                if (Files.exists(localPath)) {
                    ...
                    if (!Files.exists(parent)) {
                        ...
                        && !(looksLikeUNC(localPath.getParent()) || isWindowsNetworkMount(localPath))) {
    ...
    /**
     * COPIED FROM org.knime.core.util.FileUtils
     *
     ...
     * @since 2.11
     */
    private static URL toURL(final String path) throws MalformedURLException, InvalidPathException {
        if (path.isEmpty()) {
     ...
     * @since 2.11
     */
    private static Path resolveToPath(final URL url) throws IOException, URISyntaxException {
        if (looksLikeUNC(url)) {
    ...
    /**
     * COPIED FROM org.knime.core.util.FileUtils - but eclipse-based platform check is replace by a java (apache)-based platform check
     *
     ...
     * @since 2.11
     */
    private static boolean looksLikeUNC(final URL url) {
        // this looks like an UNC path, a real file URL does not have a host
        // Java does not handle UNC URLs correctly, see bug #5864
        return SystemUtils.IS_OS_WINDOWS && "file".equalsIgnoreCase(url.getProtocol())
