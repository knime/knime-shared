 ...
 *
 * History
 *   Feb 9, 2018 (Rene Damyon, KNIME GmbH, Berlin, Germany): created
 */
...
public enum KNIMERuntimeContext {
    ...
    private Boolean serverUsage = null;

    /**
     * Returns whether we are running in server context (in an RMI application).
     *
     * @return <code>true</code> if we are running in server context, <code>false</code> otherwise
     */
    public synchronized boolean runningInServerContext() {
        if (serverUsage == null) {
            serverUsage = false;
            Bundle myself = FrameworkUtil.getBundle(KNIMERuntimeContext.class);
            if (myself != null) {
