 ...
 * @since 6.11
 * @see org.knime.core.util.proxy.apache.ProxyCredentialsProvider
 */
public final class ProxyCredentialsProvider extends BasicCredentialsProvider {
    ...
    /**
     * Instance object for a state-less version of the credentials provider.
     * Does not use a fallback {@link URI} for auth scope parsing.
     */
    ...
    CredentialsProvider wrap(final CredentialsProvider provider) {
        return new BasicCredentialsProvider() {
            ...
            public Credentials getCredentials(final AuthScope authScope, final HttpContext context) {
                return Optional.ofNullable(provider.getCredentials(authScope, context)) //
                    .orElseGet(() -> ProxyCredentialsProvider.this.getCredentials(authScope, context));
            }
        };
    }
    ...
    private URI createURIFromAuthScope(final AuthScope authScope) {
        ...
        final var httpHost = new HttpHost(authScope.getHost(), authScope.getPort());
        try {
    ...
    public Credentials getCredentials(final AuthScope authScope, final HttpContext context) {
        ...
        return GlobalProxySearch.getCurrentFor(uri) //
            ...
            .map(cfg -> cfg.forApacheHttpClient5().getSecond().getCredentials(authScope, context)) //
            .filter(Objects::nonNull) //
            .orElseGet(() -> super.getCredentials(authScope, context));
    }
}
