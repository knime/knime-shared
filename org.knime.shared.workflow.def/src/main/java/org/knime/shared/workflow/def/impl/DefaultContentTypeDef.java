...
import org.knime.shared.workflow.def.ContentTypeDef;
...
/**
 * Content type of metadata text, not inlined because enums in superclasses are hard to represent
 *
...
public class DefaultContentTypeDef implements ContentTypeDef {

    /** this either points to a LoadException (which implements LoadExceptionTree<Void>) or to
     * a LoadExceptionTree<ContentTypeDef.Attribute> instance. */
     ...
     */
    DefaultContentTypeDef() {
        m_exceptionTree = Optional.empty();
    }

    /**
     * Builder constructor. Only for use in {@link ContentTypeDefBuilder}.
     * @param builder source
     */
    DefaultContentTypeDef(ContentTypeDefBuilder builder) {
     ...
     * Only for use in {@link ContentTypeDefBuilder}.
     ...
     */
    DefaultContentTypeDef(ContentTypeDef toCopy, final LoadException supplyException) {
        Objects.requireNonNull(supplyException);
        toCopy = Objects.requireNonNullElse(toCopy, new ContentTypeDefBuilder().build());
        
        if(toCopy instanceof DefaultContentTypeDef){
            var childTree = ((DefaultContentTypeDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public DefaultContentTypeDef(ContentTypeDef toCopy) {
        
        m_exceptionTree = Optional.empty();
    }

    // -----------------------------------------------------------------------------------------------------------------
    // Factory method to attach a LoadException to an existing ContentType
    // -----------------------------------------------------------------------------------------------------------------
    /**
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static DefaultContentTypeDef withException(ContentTypeDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadExceptionTree<?>> getLoadExceptionTree(ContentTypeDef.Attribute attribute){
        return m_exceptionTree.flatMap(t -> {
            if(t instanceof LoadException) return Optional.empty();
            // if the tree is not a leaf, it is typed to ContentTypeDef.Attribute
            return ((LoadExceptionTree<ContentTypeDef.Attribute>)t).getExceptionTree(attribute);
        });
    }
    ...
    public boolean equals(final Object o) {
        ...
        DefaultContentTypeDef other = (DefaultContentTypeDef)o;
