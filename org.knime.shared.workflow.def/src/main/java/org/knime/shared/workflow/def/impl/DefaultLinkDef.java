...
public class DefaultLinkDef implements LinkDef {

    /** this either points to a LoadException (which implements LoadExceptionTree<Void>) or to
     * a LoadExceptionTree<LinkDef.Attribute> instance. */
    ...
    /**
     * Builder constructor. Only for use in {@link LinkDefBuilder}.
     * @param builder source
     */
    DefaultLinkDef(LinkDefBuilder builder) {
     ...
     * Only for use in {@link LinkDefBuilder}.
     ...
     */
    DefaultLinkDef(LinkDef toCopy, final LoadException supplyException) {
        Objects.requireNonNull(supplyException);
        toCopy = Objects.requireNonNullElse(toCopy, new LinkDefBuilder().build());
        
        m_url = toCopy.getUrl();
        m_text = toCopy.getText();
        if(toCopy instanceof DefaultLinkDef){
            var childTree = ((DefaultLinkDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public DefaultLinkDef(LinkDef toCopy) {
        m_url = toCopy.getUrl();
        m_text = toCopy.getText();
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Factory method to attach a LoadException to an existing Link
    // -----------------------------------------------------------------------------------------------------------------
    /**
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static DefaultLinkDef withException(LinkDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadExceptionTree<?>> getLoadExceptionTree(LinkDef.Attribute attribute){
        return m_exceptionTree.flatMap(t -> {
            if(t instanceof LoadException) return Optional.empty();
            // if the tree is not a leaf, it is typed to LinkDef.Attribute
            return ((LoadExceptionTree<LinkDef.Attribute>)t).getExceptionTree(attribute);
        });
    }
    ...
    /**
     * @return The supply exception associated to url.
     */
    @JsonIgnore
    public Optional<LoadException> getUrlSupplyException(){
    	return getLoadExceptionTree(LinkDef.Attribute.URL).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public Optional<LoadException> getTextSupplyException(){
    	return getLoadExceptionTree(LinkDef.Attribute.TEXT).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public boolean equals(final Object o) {
        ...
        DefaultLinkDef other = (DefaultLinkDef)o;
        var equalsBuilder = new org.apache.commons.lang3.builder.EqualsBuilder();
        equalsBuilder.append(m_url, other.m_url);
