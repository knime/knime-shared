...
public class DefaultCipherDef implements CipherDef {
    ...
    /**
     * Builder constructor. Only for use in {@link CipherDefBuilder}.
     * @param builder source
     */
    DefaultCipherDef(CipherDefBuilder builder) {
        // TODO make immutable copies!!
        
            
        m_passwordDigest = builder.m_passwordDigest;
        m_encryptionKey = builder.m_encryptionKey;
        m_passwordHint = builder.m_passwordHint;
     ...
     * Only for use in {@link CipherDefBuilder}.
     ...
     */
    DefaultCipherDef(CipherDef toCopy, final LoadException supplyException) {
        Objects.requireNonNull(supplyException);
        toCopy = Objects.requireNonNullElse(toCopy, new CipherDefBuilder().build());
        
        m_passwordDigest = toCopy.getPasswordDigest();
        m_encryptionKey = toCopy.getEncryptionKey();
        m_passwordHint = toCopy.getPasswordHint();
        if(toCopy instanceof DefaultCipherDef){
            var childTree = ((DefaultCipherDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public DefaultCipherDef(CipherDef toCopy) {
        m_passwordDigest = toCopy.getPasswordDigest();
        m_encryptionKey = toCopy.getEncryptionKey();
        m_passwordHint = toCopy.getPasswordHint();
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Factory method to attach a LoadException to an existing Cipher
    // -----------------------------------------------------------------------------------------------------------------
    /**
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static DefaultCipherDef withException(CipherDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadExceptionTree<?>> getLoadExceptionTree(CipherDef.Attribute attribute){
        return m_exceptionTree.flatMap(t -> {
            if(t instanceof LoadException) return Optional.empty();
            // if the tree is not a leaf, it is typed to CipherDef.Attribute
            return ((LoadExceptionTree<CipherDef.Attribute>)t).getExceptionTree(attribute);
        });
    }
    ...
    /**
     * @return The supply exception associated to passwordDigest.
     */
    @JsonIgnore
    public Optional<LoadException> getPasswordDigestSupplyException(){
    	return getLoadExceptionTree(CipherDef.Attribute.PASSWORD_DIGEST).flatMap(LoadExceptionTree::getSupplyException);
    }
    
 
     
    /**
     * @return The supply exception associated to encryptionKey.
     */
    @JsonIgnore
    public Optional<LoadException> getEncryptionKeySupplyException(){
    	return getLoadExceptionTree(CipherDef.Attribute.ENCRYPTION_KEY).flatMap(LoadExceptionTree::getSupplyException);
    }
    
 
     
    /**
     * @return The supply exception associated to passwordHint.
     */
    @JsonIgnore
    public Optional<LoadException> getPasswordHintSupplyException(){
    	return getLoadExceptionTree(CipherDef.Attribute.PASSWORD_HINT).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public boolean equals(final Object o) {
        ...
        DefaultCipherDef other = (DefaultCipherDef)o;
    ...
    public int hashCode() {
        return new HashCodeBuilder()
                .append(m_passwordDigest)
                .append(m_encryptionKey)
                .append(m_passwordHint)
                .toHashCode();
    }
