...
public class FallibleAnnotationDataDef implements AnnotationDataDef {
     ...
     */
    FallibleAnnotationDataDef(AnnotationDataDefBuilder builder) {
     ...
     */
    FallibleAnnotationDataDef(AnnotationDataDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FallibleAnnotationDataDef){
            var childTree = ((FallibleAnnotationDataDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleAnnotationDataDef(AnnotationDataDef toCopy) {
        m_text = toCopy.getText();
        m_location = toCopy.getLocation();
        m_textAlignment = toCopy.getTextAlignment();
        m_borderSize = toCopy.getBorderSize();
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleAnnotationDataDef withException(AnnotationDataDef toCopy, final LoadException exception) {
     ...
     * returns the location as FallibleCoordinateDef which provides getters for the exceptions. Otherwise an empty optional.
     */
    @JsonIgnore
    public Optional<FallibleCoordinateDef> getFaultyLocation(){
    	final var location = getLocation(); 
        if(location instanceof FallibleCoordinateDef && ((FallibleCoordinateDef)location).getLoadExceptionTree().map(LoadExceptionTree::hasExceptions).orElse(false)) {
            return Optional.of((FallibleCoordinateDef)location);
        }
