...
public class FallibleRootWorkflowDef extends FallibleWorkflowDef implements RootWorkflowDef {
     ...
     */
    FallibleRootWorkflowDef(RootWorkflowDefBuilder builder) {
     ...
     */
    FallibleRootWorkflowDef(RootWorkflowDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FallibleRootWorkflowDef){
            var childTree = ((FallibleRootWorkflowDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleRootWorkflowDef(RootWorkflowDef toCopy) {
        m_name = toCopy.getName();
        m_authorInformation = toCopy.getAuthorInformation();
        m_nodes = toCopy.getNodes();
        m_connections = toCopy.getConnections();
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleRootWorkflowDef withException(RootWorkflowDef toCopy, final LoadException exception) {
     ...
     * returns the tableBackendSettings as FallibleConfigMapDef which provides getters for the exceptions. Otherwise an empty optional.
     */
    @JsonIgnore
    public Optional<FallibleConfigMapDef> getFaultyTableBackendSettings(){
    	final var tableBackendSettings = getTableBackendSettings(); 
        if(tableBackendSettings instanceof FallibleConfigMapDef && ((FallibleConfigMapDef)tableBackendSettings).getLoadExceptionTree().map(LoadExceptionTree::hasExceptions).orElse(false)) {
            return Optional.of((FallibleConfigMapDef)tableBackendSettings);
        }
    	return Optional.empty();
    }
