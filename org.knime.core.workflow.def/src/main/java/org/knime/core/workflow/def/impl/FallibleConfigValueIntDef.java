...
public class FallibleConfigValueIntDef extends FallibleConfigValueDef implements ConfigValueIntDef {

    /** this either points to a LoadException (which implements LoadExceptionTree<Void>) or to
     * a LoadExceptionTree<ConfigValueIntDef.Attribute> instance. */
    ...
    /**
     * Builder constructor. Only for use in {@link ConfigValueIntDefBuilder}.
     * @param builder source
     */
    FallibleConfigValueIntDef(ConfigValueIntDefBuilder builder) {
     ...
     * Only for use in {@link ConfigValueIntDefBuilder}.
     ...
     */
    FallibleConfigValueIntDef(ConfigValueIntDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FallibleConfigValueIntDef){
            var childTree = ((FallibleConfigValueIntDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleConfigValueIntDef(ConfigValueIntDef toCopy) {
        m_configType = toCopy.getConfigType();
        m_value = toCopy.getValue();
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Factory method to attach a LoadException to an existing ConfigValueInt
    // -----------------------------------------------------------------------------------------------------------------
    /**
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleConfigValueIntDef withException(ConfigValueIntDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadExceptionTree<?>> getLoadExceptionTree(ConfigValueIntDef.Attribute attribute){
        return m_exceptionTree.flatMap(t -> {
            if(t instanceof LoadException) return Optional.empty();
            // if the tree is not a leaf, it is typed to ConfigValueIntDef.Attribute
            return ((LoadExceptionTree<ConfigValueIntDef.Attribute>)t).getExceptionTree(attribute);
        });
    }
    ...
    public Optional<LoadException> getConfigTypeSupplyException(){
    	return getLoadExceptionTree(ConfigValueIntDef.Attribute.CONFIG_TYPE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public Optional<LoadException> getValueSupplyException(){
    	return getLoadExceptionTree(ConfigValueIntDef.Attribute.VALUE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public boolean equals(final Object o) {
        ...
        if (!(o instanceof FallibleConfigValueIntDef)) {
            return false;
        }
        FallibleConfigValueIntDef other = (FallibleConfigValueIntDef)o;
