...
public class FallibleJobManagerDef implements JobManagerDef {
     ...
     */
    FallibleJobManagerDef() {
        m_exceptionTree = Optional.empty();
    }

    /**
     * Builder constructor. Only for use in {@link JobManagerDefBuilder}.
     * @param builder source
     */
    FallibleJobManagerDef(JobManagerDefBuilder builder) {
     ...
     * Only for use in {@link JobManagerDefBuilder}.
     ...
     */
    FallibleJobManagerDef(JobManagerDef toCopy, final LoadException supplyException) {
        Objects.requireNonNull(supplyException);
        toCopy = Objects.requireNonNullElse(toCopy, new JobManagerDefBuilder().build());
        
        m_factory = toCopy.getFactory();
        m_settings = toCopy.getSettings();
        if(toCopy instanceof FallibleJobManagerDef){
            var childTree = ((FallibleJobManagerDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleJobManagerDef(JobManagerDef toCopy) {
        m_factory = toCopy.getFactory();
        m_settings = toCopy.getSettings();
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Factory method to attach a LoadException to an existing JobManager
    // -----------------------------------------------------------------------------------------------------------------
    /**
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleJobManagerDef withException(JobManagerDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadException> getFactorySupplyException(){
    	return getLoadExceptionTree(JobManagerDef.Attribute.FACTORY).flatMap(LoadExceptionTree::getSupplyException);
    }
    
 
     
    /**
     * @return The supply exception associated to settings.
     */
    @JsonIgnore
    public Optional<LoadException> getSettingsSupplyException(){
    	return getLoadExceptionTree(JobManagerDef.Attribute.SETTINGS).flatMap(LoadExceptionTree::getSupplyException);
    }
    
     
    /**
     * @return If there are {@link LoadException}s related to the {@link ConfigMapDef} returned by {@link #getSettings()}, this
     * returns the settings as FallibleConfigMapDef which provides getters for the exceptions. Otherwise an empty optional.
     */
    @JsonIgnore
    public Optional<FallibleConfigMapDef> getFaultySettings(){
    	final var settings = getSettings(); 
        if(settings instanceof FallibleConfigMapDef && ((FallibleConfigMapDef)settings).getLoadExceptionTree().map(LoadExceptionTree::hasExceptions).orElse(false)) {
            return Optional.of((FallibleConfigMapDef)settings);
        }
