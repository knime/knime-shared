...
public class FallibleNodeUIInfoDef implements NodeUIInfoDef {
     ...
     */
    FallibleNodeUIInfoDef(NodeUIInfoDefBuilder builder) {
     ...
     */
    FallibleNodeUIInfoDef(NodeUIInfoDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FallibleNodeUIInfoDef){
            var childTree = ((FallibleNodeUIInfoDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleNodeUIInfoDef(NodeUIInfoDef toCopy) {
        m_hasAbsoluteCoordinates = toCopy.hasAbsoluteCoordinates();
        m_symbolRelative = toCopy.isSymbolRelative();
        m_bounds = toCopy.getBounds();
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleNodeUIInfoDef withException(NodeUIInfoDef toCopy, final LoadException exception) {
     ...
     * returns the bounds as FallibleBoundsDef which provides getters for the exceptions. Otherwise an empty optional.
     */
    @JsonIgnore
    public Optional<FallibleBoundsDef> getFaultyBounds(){
    	final var bounds = getBounds(); 
        if(bounds instanceof FallibleBoundsDef && ((FallibleBoundsDef)bounds).getLoadExceptionTree().map(LoadExceptionTree::hasExceptions).orElse(false)) {
            return Optional.of((FallibleBoundsDef)bounds);
        }
