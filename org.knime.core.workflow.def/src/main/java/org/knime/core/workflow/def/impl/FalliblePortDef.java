...
public class FalliblePortDef implements PortDef {
     ...
     */
    FalliblePortDef(PortDefBuilder builder) {
     ...
     */
    FalliblePortDef(PortDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FalliblePortDef){
            var childTree = ((FalliblePortDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FalliblePortDef(PortDef toCopy) {
        m_index = toCopy.getIndex();
        m_portType = toCopy.getPortType();
        m_name = toCopy.getName();
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FalliblePortDef withException(PortDef toCopy, final LoadException exception) {
     ...
     * returns the portType as FalliblePortTypeDef which provides getters for the exceptions. Otherwise an empty optional.
     */
    @JsonIgnore
    public Optional<FalliblePortTypeDef> getFaultyPortType(){
    	final var portType = getPortType(); 
        if(portType instanceof FalliblePortTypeDef && ((FalliblePortTypeDef)portType).getLoadExceptionTree().map(LoadExceptionTree::hasExceptions).orElse(false)) {
            return Optional.of((FalliblePortTypeDef)portType);
        }
