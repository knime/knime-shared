...
public class FallibleConfigValueShortDef extends FallibleConfigValueDef implements ConfigValueShortDef {

    /** this either points to a LoadException (which implements LoadExceptionTree<Void>) or to
     * a LoadExceptionTree<ConfigValueShortDef.Attribute> instance. */
    ...
    /**
     * Builder constructor. Only for use in {@link ConfigValueShortDefBuilder}.
     * @param builder source
     */
    FallibleConfigValueShortDef(ConfigValueShortDefBuilder builder) {
     ...
     * Only for use in {@link ConfigValueShortDefBuilder}.
     ...
     */
    FallibleConfigValueShortDef(ConfigValueShortDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FallibleConfigValueShortDef){
            var childTree = ((FallibleConfigValueShortDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleConfigValueShortDef(ConfigValueShortDef toCopy) {
        m_configType = toCopy.getConfigType();
        m_value = toCopy.getValue();
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Factory method to attach a LoadException to an existing ConfigValueShort
    // -----------------------------------------------------------------------------------------------------------------
    /**
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleConfigValueShortDef withException(ConfigValueShortDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadExceptionTree<?>> getLoadExceptionTree(ConfigValueShortDef.Attribute attribute){
        return m_exceptionTree.flatMap(t -> {
            if(t instanceof LoadException) return Optional.empty();
            // if the tree is not a leaf, it is typed to ConfigValueShortDef.Attribute
            return ((LoadExceptionTree<ConfigValueShortDef.Attribute>)t).getExceptionTree(attribute);
        });
    }
    ...
    public Optional<LoadException> getConfigTypeSupplyException(){
    	return getLoadExceptionTree(ConfigValueShortDef.Attribute.CONFIG_TYPE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public Optional<LoadException> getValueSupplyException(){
    	return getLoadExceptionTree(ConfigValueShortDef.Attribute.VALUE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public boolean equals(final Object o) {
        ...
        if (!(o instanceof FallibleConfigValueShortDef)) {
            return false;
        }
        FallibleConfigValueShortDef other = (FallibleConfigValueShortDef)o;
