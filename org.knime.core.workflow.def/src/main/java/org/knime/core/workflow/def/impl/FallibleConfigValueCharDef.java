...
public class FallibleConfigValueCharDef extends FallibleConfigValueDef implements ConfigValueCharDef {

    /** this either points to a LoadException (which implements LoadExceptionTree<Void>) or to
     * a LoadExceptionTree<ConfigValueCharDef.Attribute> instance. */
    ...
    /**
     * Builder constructor. Only for use in {@link ConfigValueCharDefBuilder}.
     * @param builder source
     */
    FallibleConfigValueCharDef(ConfigValueCharDefBuilder builder) {
     ...
     * Only for use in {@link ConfigValueCharDefBuilder}.
     ...
     */
    FallibleConfigValueCharDef(ConfigValueCharDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FallibleConfigValueCharDef){
            var childTree = ((FallibleConfigValueCharDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleConfigValueCharDef(ConfigValueCharDef toCopy) {
        m_configType = toCopy.getConfigType();
        m_value = toCopy.getValue();
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Factory method to attach a LoadException to an existing ConfigValueChar
    // -----------------------------------------------------------------------------------------------------------------
    /**
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleConfigValueCharDef withException(ConfigValueCharDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadExceptionTree<?>> getLoadExceptionTree(ConfigValueCharDef.Attribute attribute){
        return m_exceptionTree.flatMap(t -> {
            if(t instanceof LoadException) return Optional.empty();
            // if the tree is not a leaf, it is typed to ConfigValueCharDef.Attribute
            return ((LoadExceptionTree<ConfigValueCharDef.Attribute>)t).getExceptionTree(attribute);
        });
    }
    ...
    public Optional<LoadException> getConfigTypeSupplyException(){
    	return getLoadExceptionTree(ConfigValueCharDef.Attribute.CONFIG_TYPE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public Optional<LoadException> getValueSupplyException(){
    	return getLoadExceptionTree(ConfigValueCharDef.Attribute.VALUE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public boolean equals(final Object o) {
        ...
        if (!(o instanceof FallibleConfigValueCharDef)) {
            return false;
        }
        FallibleConfigValueCharDef other = (FallibleConfigValueCharDef)o;
