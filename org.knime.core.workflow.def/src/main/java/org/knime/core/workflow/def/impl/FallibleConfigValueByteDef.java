...
public class FallibleConfigValueByteDef extends FallibleConfigValueDef implements ConfigValueByteDef {

    /** this either points to a LoadException (which implements LoadExceptionTree<Void>) or to
     * a LoadExceptionTree<ConfigValueByteDef.Attribute> instance. */
    ...
    /**
     * Builder constructor. Only for use in {@link ConfigValueByteDefBuilder}.
     * @param builder source
     */
    FallibleConfigValueByteDef(ConfigValueByteDefBuilder builder) {
     ...
     * Only for use in {@link ConfigValueByteDefBuilder}.
     ...
     */
    FallibleConfigValueByteDef(ConfigValueByteDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FallibleConfigValueByteDef){
            var childTree = ((FallibleConfigValueByteDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleConfigValueByteDef(ConfigValueByteDef toCopy) {
        m_configType = toCopy.getConfigType();
        m_value = toCopy.getValue();
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Factory method to attach a LoadException to an existing ConfigValueByte
    // -----------------------------------------------------------------------------------------------------------------
    /**
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleConfigValueByteDef withException(ConfigValueByteDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadExceptionTree<?>> getLoadExceptionTree(ConfigValueByteDef.Attribute attribute){
        return m_exceptionTree.flatMap(t -> {
            if(t instanceof LoadException) return Optional.empty();
            // if the tree is not a leaf, it is typed to ConfigValueByteDef.Attribute
            return ((LoadExceptionTree<ConfigValueByteDef.Attribute>)t).getExceptionTree(attribute);
        });
    }
    ...
    public Optional<LoadException> getConfigTypeSupplyException(){
    	return getLoadExceptionTree(ConfigValueByteDef.Attribute.CONFIG_TYPE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public Optional<LoadException> getValueSupplyException(){
    	return getLoadExceptionTree(ConfigValueByteDef.Attribute.VALUE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public boolean equals(final Object o) {
        ...
        if (!(o instanceof FallibleConfigValueByteDef)) {
            return false;
        }
        FallibleConfigValueByteDef other = (FallibleConfigValueByteDef)o;
