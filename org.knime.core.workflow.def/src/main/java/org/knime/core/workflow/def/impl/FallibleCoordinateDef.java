...
public class FallibleCoordinateDef implements CoordinateDef {
     ...
     */
    FallibleCoordinateDef(CoordinateDefBuilder builder) {
     ...
     */
    FallibleCoordinateDef(CoordinateDef toCopy, final LoadException supplyException) {
        Objects.requireNonNull(supplyException);
        toCopy = Objects.requireNonNullElse(toCopy, new CoordinateDefBuilder().build());
        
        m_x = toCopy.getX();
        m_y = toCopy.getY();
        if(toCopy instanceof FallibleCoordinateDef){
            var childTree = ((FallibleCoordinateDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleCoordinateDef(CoordinateDef toCopy) {
        m_x = toCopy.getX();
        m_y = toCopy.getY();
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleCoordinateDef withException(CoordinateDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadExceptionTree<?>> getLoadExceptionTree(CoordinateDef.Attribute attribute){
        return m_exceptionTree.flatMap(t -> {
            ...
            return ((LoadExceptionTree<CoordinateDef.Attribute>)t).getExceptionTree(attribute);
        });
    }
