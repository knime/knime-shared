...
public class FallibleStandaloneDef implements StandaloneDef {
     ...
     */
    FallibleStandaloneDef(StandaloneDefBuilder builder) {
     ...
     */
    FallibleStandaloneDef(StandaloneDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FallibleStandaloneDef){
            var childTree = ((FallibleStandaloneDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleStandaloneDef(StandaloneDef toCopy) {
        m_creator = toCopy.getCreator();
        m_contents = toCopy.getContents();
        m_contentType = toCopy.getContentType();
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleStandaloneDef withException(StandaloneDef toCopy, final LoadException exception) {
     ...
     * returns the creator as FallibleCreatorDef which provides getters for the exceptions. Otherwise an empty optional.
     */
    @JsonIgnore
    public Optional<FallibleCreatorDef> getFaultyCreator(){
    	final var creator = getCreator(); 
        if(creator instanceof FallibleCreatorDef && ((FallibleCreatorDef)creator).getLoadExceptionTree().map(LoadExceptionTree::hasExceptions).orElse(false)) {
            return Optional.of((FallibleCreatorDef)creator);
        }
