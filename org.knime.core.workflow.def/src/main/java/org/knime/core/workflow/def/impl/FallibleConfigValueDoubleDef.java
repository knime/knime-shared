...
public class FallibleConfigValueDoubleDef extends FallibleConfigValueDef implements ConfigValueDoubleDef {

    /** this either points to a LoadException (which implements LoadExceptionTree<Void>) or to
     * a LoadExceptionTree<ConfigValueDoubleDef.Attribute> instance. */
    ...
    /**
     * Builder constructor. Only for use in {@link ConfigValueDoubleDefBuilder}.
     * @param builder source
     */
    FallibleConfigValueDoubleDef(ConfigValueDoubleDefBuilder builder) {
     ...
     * Only for use in {@link ConfigValueDoubleDefBuilder}.
     ...
     */
    FallibleConfigValueDoubleDef(ConfigValueDoubleDef toCopy, final LoadException supplyException) {
        ...
        if(toCopy instanceof FallibleConfigValueDoubleDef){
            var childTree = ((FallibleConfigValueDoubleDef)toCopy).getLoadExceptionTree();                
     ...
     * @param toCopy source
     */
    public FallibleConfigValueDoubleDef(ConfigValueDoubleDef toCopy) {
        m_configType = toCopy.getConfigType();
        m_value = toCopy.getValue();
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Factory method to attach a LoadException to an existing ConfigValueDouble
    // -----------------------------------------------------------------------------------------------------------------
    /**
     ...
     * @param exception supply exception, not null (use the copy constructor otherwise)
     */
    static FallibleConfigValueDoubleDef withException(ConfigValueDoubleDef toCopy, final LoadException exception) {
    ...
    public Optional<LoadExceptionTree<?>> getLoadExceptionTree(ConfigValueDoubleDef.Attribute attribute){
        return m_exceptionTree.flatMap(t -> {
            if(t instanceof LoadException) return Optional.empty();
            // if the tree is not a leaf, it is typed to ConfigValueDoubleDef.Attribute
            return ((LoadExceptionTree<ConfigValueDoubleDef.Attribute>)t).getExceptionTree(attribute);
        });
    }
    ...
    public Optional<LoadException> getConfigTypeSupplyException(){
    	return getLoadExceptionTree(ConfigValueDoubleDef.Attribute.CONFIG_TYPE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public Optional<LoadException> getValueSupplyException(){
    	return getLoadExceptionTree(ConfigValueDoubleDef.Attribute.VALUE).flatMap(LoadExceptionTree::getSupplyException);
    }
    ...
    public boolean equals(final Object o) {
        ...
        if (!(o instanceof FallibleConfigValueDoubleDef)) {
            return false;
        }
        FallibleConfigValueDoubleDef other = (FallibleConfigValueDoubleDef)o;
